...
public:
    ...
    void add_window(Window_base *pWindow, Window_listen_status statu, SDL_Window *pSDL_window = nullptr, SDL_Renderer *pRenderer = nullptr);
    void remove_window(Window_base *pWindow);
    ...
    void patch_event_to(Window_base *pWnd, const SDL_Event &event, Window_listen_status statu);
    void patch_event_to_all(const SDL_Event &event, Window_listen_status statu);
    void update_all_window();
    void update_window(Window_base *pWindow);
    void update_window(uint32_t window_no);
    void update_window_statu(Window_base *pWindow, Window_listen_status statu);
    Window_base *get_window(Uint32 window_no);
    void check_and_delete_window_data(Uint32 window_no);
    void update_texture_count(SDL_Renderer *pRenderer, int count);
    ~Window_manager();
    SDL_Window *get_sdl_window(const Window_base *pWindow);
    SDL_Renderer *get_sdl_renderer(const Window_base *pWindow);
...
private:
    struct Window_data;
    std::map<Uint32, std::pair<Window_base *, Window_data>> window_map;
    std::map<SDL_Renderer *, SDL_Texture *> target_map;
    void push_event(Window_base *pw, const SDL_Event &event);
